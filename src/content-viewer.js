/**
 * UiPath XAML Viewer — Viewer lifecycle and rendering module.
...
${outlineHtml}
<div class="footer">Generated by UiPath XAML Viewer</div>
</body></html>`;
  ...
  function renderViewer() {
    ...
    if (typeof rendered === 'string') {
      viewer.remove();
      renderMessageViewer('UiPath XAML Viewer', rendered, 'warning');
      return;
    }
    viewer.innerHTML = `
      <div class="uxv-header">
  ...
  async function toggleViewer() {
    ...
    try {
      ...
      if (!xaml) {
        renderMessageViewer('UiPath XAML Viewer', 'Could not retrieve XAML from this page. The page layout may have changed or the raw file is unavailable.');
        resetBtn(btn);
        return;
      }
      ...
      if (state.currentParsed.error) {
        renderMessageViewer('UiPath XAML Viewer', state.currentParsed.error);
        resetBtn(btn);
        return;
      }
      ...
      renderMessageViewer('UiPath XAML Viewer', err.message || 'Unexpected viewer error.');
      resetBtn(btn);
    } finally {

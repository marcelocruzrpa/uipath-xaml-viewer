/**
 * UiPath XAML Viewer — Service worker for dynamic host registration.
 *
 * Reads the list of known GitHub Enterprise and self-hosted GitLab hosts
 * from storage and registers content scripts dynamically so the extension
 * works on custom instances. Host registration is independent of whether a
 * token has been saved — public instances work without configuration.
 */
...
async function getGitLabHosts() {
  const result = await chrome.storage.local.get('gitlab_hosts');
  const hosts = result.gitlab_hosts;
  if (!Array.isArray(hosts)) return [];
  return hosts.filter((h) => typeof h === 'string' && h !== 'gitlab.com');
}

async function updateGitLabContentScripts() {
  const hosts = await getGitLabHosts();
  ...
  const activeIds = new Set(hosts.map((h) => `uxv-gl-${h}`));
  for (const script of registered) {
    if (script.id.startsWith('uxv-gl-') && !activeIds.has(script.id)) {
      try { await chrome.scripting.unregisterContentScripts({ ids: [script.id] }); } catch (_e) { /* already gone */ }
    }
  }

  for (const host of hosts) {
    const id = `uxv-gl-${host}`;
    const pattern = `*://${host}/*`;
    ...
    try {
      ...
      console.info('[UXV] Registered GitLab content scripts for:', host);
    } catch (e) {
      console.warn(`[UXV] Failed to register content scripts for ${host}:`, e.message);
    }
  }
...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  if (changes.gitlab_hosts) updateGitLabContentScripts();
});

// Register on install/update
chrome.runtime.onInstalled.addListener(() => {
  updateGheContentScripts();
  updateGitLabContentScripts();
});
